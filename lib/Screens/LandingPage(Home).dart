...
import 'package:egybest/Screens/genresAll.dart';
...
class _LandingState extends State<Landing> {
  ...
  // changeData(width, height, index) async {
  //   genreMovies.clear();

  //   var tv = genresDataAll[index]['_links']['wp:post_type'][0]['href'];
  //   var movie = genresDataAll[index]['_links']['wp:post_type'][1]['href'];
  ...
  //   return moviesList(
  //       width, height, "${genresDataAll[index]['name']}", genreMovies);
  // }
  ...
  genresData() async {
    try {
      var data = await http.get("https://xegybest.com/wp-json/wp/v2/genres");
      if (data.statusCode == 200) {
        genresDataAll = jsonDecode(data.body);
        print(genresDataAll);

        //  changeData(0);
      }

      return genresDataAll;
    } catch (e) {
      print(e);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return
      ...
      child: DefaultTabController(
          length: genresDataAll.length == null ? 0 : genresDataAll.length,
          child: SafeArea(
            child: Scaffold(
              ...
              appBar: AppBar(
                ...
                bottom: TabBar(
                  ...
                  isScrollable: true,
                  tabs: [
                    for (var i = 0; i < genresDataAll.length; i++)
                      Tab(
                        text: "${genresDataAll[i]['name']}",
                      )
                  ],
                ),
                actions: [
      ...
      children: [
              ...
              child: MaterialButton(
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => Grid(moviesData)));
                },
                child: Text(
                  "SEE ALL",
                  style: TextStyle(
        ...
        Container(
          ...
          child: ListView.builder(
            shrinkWrap: true,
            scrollDirection: Axis.horizontal,
            itemCount: moviesData.length == 0 ? 0 : moviesData.length,
            itemBuilder: (BuildContext context, int i) {
              return InkWell(
                onTap: () {
                  Navigator.push(
